...
var (
    ...
    encoding           = flag.String("encoding", "json", "Server export encoding")
)
...
func ParseQuery(s, delimiter string) query.Query {
	...
	for _, qItem := range queries {
        ...
        for len(qItem) > 0 {
            ...
            if (elem_delim_offset > param_delim_offset) {
                ...
                if (strings.Index(qItem, "]") + 2 >= len(qItem)) {
                    ...
                    qItem = qItem[strings.Index(qItem, "]") + 2: len(qItem)]
                }
            } else if (param_delim_offset > elem_delim_offset) {
		...
		q.Queries = append(q.Queries, elems)
	}
	return q
}
